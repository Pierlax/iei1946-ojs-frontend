...
// Reads the article from OJS via /api/ojs. No static fallback.
// ============================================================
...
import { useArticle, useRecentArticles } from "@/hooks/useOJS";
import {
  ArrowLeft,
  FileText,
  Download,
...
export default function ArticleDetail() {
  ...
  const { data: recentArticlesRaw } = useRecentArticles(5);
          ...
          <div className="max-w-2xl mx-auto text-center">
            ...
            <p className="text-gray-500 mb-6">
              {error?.message?.includes("404")
                ? "This article does not exist on the journal's OJS server."
                : error
  ...
  const article = liveArticle as unknown as Article;
  const recentArticles = ((recentArticlesRaw as unknown as Article[]) || []).filter(
    (a) => String(a.id) !== id,
  );

  const volumeInfo =
    article.volume || article.issue || article.year
      ? `Volume ${article.volume || "?"}${article.issue ? `, Issue ${article.issue}` : ""}${article.year ? `, ${article.month ? article.month + " " : ""}${article.year}` : ""}`
      : "";
      ...
      <div className="bg-gray-50 border-b border-gray-200">
        <div className="container py-3">
          <nav className="flex items-center gap-2 text-xs text-gray-500">
            <Link href="/" className="hover:text-[#009e8e] transition-colors">Home</Link>
            <ChevronRight size={12} />
            <Link href="/review" className="hover:text-[#009e8e] transition-colors">Review</Link>
            <ChevronRight size={12} />
            <span className="text-gray-700 truncate max-w-xs font-medium">{article.title}</span>
          </nav>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
            <aside className="lg:col-span-1 order-2 lg:order-1 space-y-5">
              <div className="bg-[#1b3a5c] rounded p-5 text-white">
                ...
                <p className="text-white/70 text-xs mt-2 leading-relaxed">No APCs. Free submission.</p>
                <a
            ...
            <div className="lg:col-span-3 order-1 lg:order-2">
              ...
              <div className="mb-4">
                {article.authors.map((author, i) => (
                  <span key={`${author.name}-${i}`} className="text-sm text-[#1b3a5c]/80">
                    {author.name}
                    {author.affiliation && (
              ...
              {(volumeInfo || article.pages || article.doi || article.keywords?.length || article.jel?.length) && (
